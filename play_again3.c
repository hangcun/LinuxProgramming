...
void set_nodelay_mode() {
	 ...
	 *  method: use fcntl to get bits
	 *   notes: tcsetattr() will do something similar, but it is conplicated
	 */
	int termflags;
	termflags = fcntl(0, F_GETFL);			/* read current settings	*/
	termflags |= O_NDELAY;					/* flip on nodelay bits		*/
	fcntl(0, F_SETFL, termflags);			/* install settings			*/
}
